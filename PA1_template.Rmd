```{r setting_english, echo = FALSE, results = 'hide'}
...
Analizing the results, we observe that during the 2 months the experiment lasted, an average of `r mean_t_steps`, were taken per day, similarly, the median of daily steps taken per day was `r median_t_steps`, however, on the least active day, only `r min_t_steps` where taken (just from bed to bathroom!), in contrast,  the maximum steps recorded on a single day was `r max_t_steps`.
...
```{r estimating_missing}
totalNA <- sum(is.na(mydata$steps))
percentageNA <- format(100*totalNA/nrow(mydata),digits = 3,nsmall = 1)
```

In this experiment, a total of `r totalNA` cases were reported as `NA`, that is, `r percentageNA` % of total cases, which is not a negligable proportion, therefore, we need to find out a way to recover at least most of the missing data. 
...
Analizing the results, we observe that during the 2 months the experiment lasted, an average of `r mean_t_steps2`, were taken per day, which differes from the median of daily steps (`r median_t_steps2`), this difference is due to the imputed data, which pulled the mean towards the left. On the other hand, the minimum of steps in these period was `r min_t_steps2` whereas the maximum recorded was `r max_t_steps2`.
...
Finally, the next table summarizes the differences between raw data and imputed data
```{r table1}
library(knitr)
kable(c("hola", "hola2"),format = "markdown")
```
col1 <- c(c1 = 
|   | Mean | Median | Min | Max |
|---|:-:|:-:|:-:|:-:|
| With Missing values  | `r mean_t_steps`  | `r median_t_steps` | `r min_t_steps`  | `r max_t_steps`  |
| With Imputed values  | `r mean_t_steps2`  | `r median_t_steps2`  | `r min_t_steps2` | `r max_t_steps2` |
